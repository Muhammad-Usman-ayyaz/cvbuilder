...
import { improveResume } from '../api/atsApi';
                ...
                {change.type === 'skills-new' || change.type === 'skills-add' ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div>
                            ...
                            <p className="text-text-secondary bg-white rounded-md p-2.5 border border-border whitespace-pre-wrap">
 ...
 * }} props
 */
export default function ImproveResumePanel({ isOpen, onClose, resumeId, resume, jobDescription, currentAnalysis, saveResume, onUsed }) {
    const [status, setStatus] = useState('idle'); // idle | loading | ready | error | applying | applied
            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-card border border-border rounded-xl shadow-xl"
                    >
                        <div className="flex items-center justify-between p-5 border-b border-border shrink-0">
                            <div className="flex items-center gap-2">
                                <span className="material-symbols-outlined text-primary text-xl">auto_awesome</span>
                                <h2 id="improve-resume-title" className="text-base font-semibold text-text-primary">
                                    Improve This Resume
                                </h2>
                            </div>
                            <button
                                ...
                                aria-label="Close"
                                className="p-1.5 rounded-md text-text-secondary hover:text-text-primary hover:bg-bg-main transition-colors disabled:opacity-50"
                            >
                        ...
                        {(status === 'ready' || status === 'applying') && changes.length > 0 && (
                            <div className="flex items-center justify-end gap-2 p-5 border-t border-border shrink-0">
                                <Button type="button" variant="ghost" onClick={onClose} disabled={status === 'applying'}>
                                    Discard
                                </Button>
